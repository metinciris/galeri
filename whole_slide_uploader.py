    ...
    *,
    ...
    for job in successful_uploads:
        try:
            ...
            failed.append((job, str(exc)))
            emit("error", str(exc), repo=job.repo_name, stage="complete_error")
            continue
    ...
    emit(
        "batch_done",
        f"Tamamlandi: {len(completed)} basarili, {len(failed)} hatali",
...
def launch_gui() -> int:
    ...
    class UploaderApp(tk.Tk):
        def __init__(self) -> None:
            super().__init__()
            self.title("Whole Slide GitHub Uploader")
        ...
        def _build_ui(self) -> None:
            ...
            gallery_box = ttk.LabelFrame(outer, text="Ana Galeri", padding=10)
            gallery_box.pack(fill="x", pady=(0, 8))
            ttk.Label(gallery_box, text="Baslik:").grid(row=0, column=0, sticky="w", padx=(0, 6))
            self.gallery_title_var = tk.StringVar(value="Slide Gallery")
            self.gallery_title_entry = ttk.Entry(gallery_box, textvariable=self.gallery_title_var)
            self.gallery_title_entry.grid(row=0, column=1, sticky="ew")
            ttk.Label(gallery_box, text="Aciklama:").grid(row=1, column=0, sticky="nw", padx=(0, 6), pady=(6, 0))
            self.gallery_desc = tk.Text(gallery_box, height=2, wrap="word")
            self.gallery_desc.grid(row=1, column=1, sticky="ew", pady=(6, 0))
            gallery_buttons = ttk.Frame(gallery_box)
            gallery_buttons.grid(row=0, column=2, rowspan=2, sticky="ns", padx=(10, 0))
            self.gallery_reload_btn = ttk.Button(gallery_buttons, text="GitHub'dan Oku", command=self.reload_gallery_settings)
            self.gallery_reload_btn.pack(fill="x", pady=(0, 5))
            self.gallery_save_btn = ttk.Button(gallery_buttons, text="Kaydet ve Yayinla", command=self.publish_gallery_settings)
            self.gallery_save_btn.pack(fill="x")
            self.gallery_status_var = tk.StringVar(value="")
            ttk.Label(gallery_box, textvariable=self.gallery_status_var).grid(row=2, column=1, sticky="w", pady=(4, 0))
            ...
            self.tree = ttk.Treeview(left, columns=("repo", "status"), show="tree headings", height=10)
            self.tree.heading("#0", text="SVS")
            self.tree.heading("repo", text="Repo")
            self.tree.heading("status", text="Durum")
            self.tree.column("#0", width=260, minwidth=180)
            self.tree.column("repo", width=110, anchor="center")
        ...
        def set_busy(self, busy: bool) -> None:
            ...
            self.gallery_reload_btn.configure(state=normal)
            self.gallery_save_btn.configure(state=normal)
        ...
        def _startup_worker(self) -> None:
            try:
                ...
                try:
                    ...
                    self.events.put({"kind": "warning", "message": f"Galeri baslik/aciklama okunamadi: {exc}"})
        ...
        def populate_jobs(self, jobs: List[SlideJob]) -> None:
            ...
            for job in jobs:
                ...
                self.tree.insert("", "end", iid=job.repo_name, text=job.svs_path.name, values=(job.repo_name, stage_text(stage, job.prepared)))
        ...
        def start_upload(self) -> None:
            ...
            if unprepared:
                ...
                return
            title = self.gallery_title_var.get().strip() or "Slide Gallery"
            desc = self.gallery_desc.get("1.0", "end").strip()
        ...
        def reload_gallery_settings(self) -> None:
            if self.busy:
                return
            self.set_busy(True)
            self.gallery_status_var.set("GitHub'dan okunuyor...")
            threading.Thread(target=self._gallery_reload_worker, daemon=True).start()

        def _gallery_reload_worker(self) -> None:
            try:
                title, desc = load_remote_gallery_settings()
                self.events.put({"kind": "gallery_settings_loaded", "gallery_title": title, "gallery_description": desc})
            except Exception as exc:
                self.events.put({"kind": "gallery_action_error", "message": str(exc)})

        def publish_gallery_settings(self) -> None:
            if self.busy:
                return
            title = self.gallery_title_var.get().strip()
            desc = self.gallery_desc.get("1.0", "end").strip()
            if not title:
                messagebox.showerror("Ana Galeri", "Galeri basligi bos olamaz.")
                return
            self.set_busy(True)
            self.gallery_status_var.set("GitHub'a yaziliyor ve canli yayin dogrulaniyor...")
            threading.Thread(target=self._gallery_publish_worker, args=(title, desc), daemon=True).start()

        def _gallery_publish_worker(self, title: str, desc: str) -> None:
            try:
                sync_gallery(set(), gallery_title=title, gallery_description=desc)
                wait_for_gallery_live(set(), expected_title=title)
                self.events.put({"kind": "gallery_publish_done", "message": "Ana galeri baslik ve aciklamasi yayinda."})
            except Exception as exc:
                self.events.put({"kind": "gallery_action_error", "message": str(exc)})
        ...
        def update_tree_job(self, job: SlideJob) -> None:
            ...
            if job.state.get("gallery_verified") and job.state.get("archived"):
                stage = "complete"
            self.tree.item(job.repo_name, text=job.svs_path.name, values=(job.repo_name, stage_text(stage, job.prepared)))
            ...
            try:
                while True:
                    ...
                    if kind == "startup_loaded":
                        self.gallery_title_var.set(event.get("gallery_title") or "Slide Gallery")
                        self.gallery_desc.delete("1.0", "end")
                        self.gallery_desc.insert("1.0", event.get("gallery_description") or "")
                    ...
                    if kind == "startup_error":
                        ...
                        continue
                    if kind == "gallery_settings_loaded":
                        self.gallery_title_var.set(event.get("gallery_title") or "Slide Gallery")
                        self.gallery_desc.delete("1.0", "end")
                        self.gallery_desc.insert("1.0", event.get("gallery_description") or "")
                        self.gallery_status_var.set("GitHub'dan guncellendi.")
                        self.set_busy(False)
                        continue
                    if kind == "gallery_publish_done":
                        self.gallery_status_var.set(message)
                        self.set_busy(False)
                        messagebox.showinfo("Ana Galeri", message)
                        continue
                    if kind == "gallery_action_error":
                        self.gallery_status_var.set("Hata: " + message)
                        self.set_busy(False)
                        messagebox.showerror("Ana Galeri", message)
                        continue
                    ...
                    if repo and repo in self.job_by_repo:
                        ...
                        if kind == "cleanup_available":
                            job.state["gallery_verified"] = True
                            job.state["archived"] = True
                    ...
                    if kind == "batch":
                        ...
                        for job in self.jobs:
                            if job.state.get("gallery_verified"):
                                job.state["archived"] = True
                            self.update_tree_job(job)
                            card = self.cards.get(job.repo_name)
                            if card:
                                card.progress_value = 100 if job.state.get("gallery_verified") else card.progress_value
                                card.refresh_from_job()
                        if failed:
                            messagebox.showwarning(
                                "Yukleme tamamlandi",
                                f"{completed} basarili, {failed} hata. Hata alan SVS ve durum dosyalari yüklenecek klasorunde kaldi; sonraki calistirmada devam eder.",
                            )
                        else:
